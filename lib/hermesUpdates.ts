...
export const hermesUpdates: HermesUpdate[] = [
  ...
  {
    date: "2026-05-01",
    title: "Agent guardrails 강화, 지연 세션 생성, /goal 크로스턴 목표 및 DeepSeek V4 Pro 보정",
    category: "Agent 안정성",
    summary:
      "도구 호출 반복 루프를 감지하는 guardrail이 warning-first 방식으로 추가되고, display 레벨의 도구 실패 감지 의미를 보존하도록 수정됐습니다. 세션 생성을 첫 메시지까지 지연시켜 불필요한 DB row를 방지하며, 크로스턴 목표를 유지하는 /goal 기능(Ralph loop)도 도입됐습니다. DeepSeek V4 Pro thinking mode에서 reasoning_content placeholder가 비어있지 않도록 보정됐습니다.",
    commits: [
      {
        sha: "58b8996",
  ...
  {
    date: "2026-05-01",
    title: "Gateway snapshot·슬래시 알림 TTL·세션 분기 메타데이터 보존 및 State replay finish_reason",
    category: "Gateway / State",
    summary:
      "Gateway 콜백 snapshot 시점이 agent bind 이후로 조정되고, slash command 시스템 알림이 TTL 기반으로 자동 삭제됩니다. 세션 분기 시 assistant 메타데이터를 보존하고, queued event 병합 시 document type을 유지하도록 수정됐습니다. conversation replay에 finish_reason이 포함되며, tool_progress_command와 StreamingConfig의 boolean 값이 안전하게 보정됩니다.",
    commits: [
      {
        sha: "8d7500d",
  ...
  {
    date: "2026-05-01",
    title: "Config boolean 보정, atomic 저장, update --yes 플래그 및 YOLO 모드 파싱 강화",
    category: "Config / Auth",
    summary:
      "문자열로 들어오는 설정값을 실제 boolean으로 보정하고, provider config 저장을 atomic write로 전환해 설정 파일 손상을 방지합니다. hermes update에 --yes/-y 플래그가 추가돼 대화형 프롬프트를 건너뛸 수 있으며, skill config scan 실패 시 update가 중단되지 않도록 수정됐습니다. YOLO 모드 환경변수 파싱이 quoted-bool 문자열에 대해 강화됐습니다.",
    commits: [
      {
        sha: "27ec74c",
  ...
  {
    date: "2026-05-01",
    title: "Discord·Yuanbao·Telegram 도구 수정, Terminal sudo 건너뛰기 및 Browser Supervisor 헬스 체크",
    category: "Tools / MCP / Plugins",
    summary:
      "Discord 툴의 capability 캐시가 토큰별로 분리되고, Yuanbao 그룹 slash command에 owner identity 검사가 적용됩니다. Telegram에서 DM topic 생성 후 seed message를 전송하도록 수정됐으며, terminal에서 로컬 NOPASSWD sudo가 가능하면 sudo 프롬프트를 건너뜁니다. browser_supervisor에서 캐시된 supervisor를 반환하기 전 thread와 loop 상태를 확인하도록 수정됐습니다.",
    commits: [
      {
        sha: "fa7b0b0",
  ...
  {
    date: "2026-05-01",
    title: "문서 사이트: 사용자 스토리, llms.txt, /goal 페이지, TUI 자산 빌드 방지 및 단축키 정리",
    category: "Docs / UI",
    summary:
      "웹사이트에 User Stories 및 Use Cases 콜라주 페이지가 추가되고, 에이전트 친화적 수집을 위한 llms.txt와 llms-full.txt가 발행됐습니다. /goal 기능 문서 페이지가 추가됐으며, TUI가 불필요하게 자산을 재빌드하지 않도록 수정되고 TUI 리뷰 피드백(정렬 및 숫자 단축키)이 반영됐습니다.",
    commits: [
      {
        sha: "a2a3268",
